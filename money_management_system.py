...
def calculate_remaining():
    ...
    draw_piechart([int(entry.get()) for entry in expense_entries])

def draw_piechart(expenses):
    figure = plt.Figure(figsize=(5,5), dpi=100)
    ax = figure.add_subplot(111)
    pie = FigureCanvasTkAgg(figure, root)
    pie.get_tk_widget().grid(row=4, column=1)
