...
def calculate_remaining():
    ...
    save_to_spreadsheet(total_income, [int(entry.get()) for entry in expense_entries], remaining)
...
def save_to_spreadsheet(total_income, expenses, remaining):
    # Create a DataFrame
    df = pd.DataFrame({
        'Income/Expense': ['Salary', 'Additional Income'] + categories + ['Remaining'],
        'Amount': [salary_entry.get(), additional_income_entry.get()] + expenses + [remaining]
    })
    # Ask the user to input the name of the file and choose the location to save the file
